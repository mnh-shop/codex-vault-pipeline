#!/usr/bin/env python3
"""Phase 5 — Knowledge Note Candidate Generator (AgentField core batch).

Generates candidate knowledge notes under wiki/_candidates/ for sub-topics
of github:Agent-Field/agentfield that are not already covered by the 2 existing
wiki notes under wiki/agentfield/ (agentfield.md, ecosystem.md).

Sub-topics covered (all under github:Agent-Field/agentfield):
  1. agentfield-sdks         — sdk/ subdir (6 files: Python, Go, TypeScript)
  2. agentfield-deployments  — deployments/ subdir (5 files: docker, helm,
                                kubernetes, railway, README)
  3. agentfield-integrations — integrations/ subdir (12 files: databricks,
                                linear, sentry, snowflake)

Excluded from this batch (per user instruction):
  - The 7 AgentField example-agent repositories (SWE-AF, reels-af, plandb,
    cloudsecurity-af, af-deep-research, pr-af, sec-af). These are separate
    repositories with their own source records; they are NOT part of
...
Each candidate:
  ...
  - individual source_role=core (this batch is AgentField core)
...
Usage:
    python3 _phase5_candidate_agentfield_core.py \
...
# Candidate definitions. file_path_filter scopes occurrences to specific subdirs
# of agentfield/ to avoid overlap with the 2 existing wiki/agentfield/*.md notes.
CANDIDATE_DEFS = [
    {
        "slug": "agentfield-sdks",
        "title": "AgentField SDKs — Python, Go, and TypeScript",
        "source_id": SOURCE_ID,
        "file_path_filter": "agentfield/sdk/",
        "scope_covers": (
            "Sub-topics of github:Agent-Field/agentfield covered by the "
            "`sdk/` subdirectory: the Python SDK, Go SDK, and TypeScript SDK. "
            "Backed by 6 documentation files in the Phase 3 manifest "
            "(sdk/python, sdk/go, sdk/typescript)."
        ),
        "scope_excludes": (
            "The AgentField project overview (covered by wiki/agentfield/agentfield.md), "
            "the AgentField ecosystem of separate repositories "
            "(covered by wiki/agentfield/ecosystem.md), and the 7 AgentField "
            "example-agent repositories (SWE-AF, reels-af, plandb, "
            "cloudsecurity-af, af-deep-research, pr-af, sec-af) which are "
            "out of scope for this batch per user instruction. Production "
            "deployment options are covered by a separate candidate "
            "(agentfield-deployments). External integrations (databricks, "
            "linear, sentry, snowflake) are covered by a separate candidate "
            "(agentfield-integrations). Control plane internals and the "
            "top-level docs/ subdirectory are partially covered by the "
            "existing wiki/agentfield/agentfield.md note."
        ),
        "source_role": "core",
    ...
    {
        "slug": "agentfield-deployments",
        "title": "AgentField Deployments — Docker, Helm, Kubernetes, and Railway",
        "source_id": SOURCE_ID,
        "file_path_filter": "agentfield/deployments/",
        "scope_covers": (
            "Sub-topics of github:Agent-Field/agentfield covered by the "
            "`deployments/` subdirectory: production deployment options "
            "including Docker, Helm, Kubernetes, and Railway. Backed by 5 "
            "documentation files in the Phase 3 manifest (deployments/README, "
            "deployments/docker, deployments/helm, deployments/kubernetes, "
            "deployments/railway)."
        ),
        "scope_excludes": (
            "The AgentField project overview (covered by wiki/agentfield/agentfield.md), "
            ...
            "example-agent repositories (SWE-AF, reels-af, plandb, "
            "cloudsecurity-af, af-deep-research, pr-af, sec-af) which are "
            "out of scope for this batch per user instruction. SDKs (Python, "
            "Go, TypeScript) are covered by a separate candidate "
            "(agentfield-sdks). External integrations (databricks, linear, "
            "sentry, snowflake) are covered by a separate candidate "
            "(agentfield-integrations). The control plane internals and "
            "top-level docs/ subdirectory are partially covered by the "
            "existing wiki/agentfield/agentfield.md note."
        ),
        "source_role": "core",
    ...
    {
        "slug": "agentfield-integrations",
        "title": "AgentField Integrations — Databricks, Linear, Sentry, and Snowflake",
        "source_id": SOURCE_ID,
        "file_path_filter": "agentfield/integrations/",
        "scope_covers": (
            "Sub-topics of github:Agent-Field/agentfield covered by the "
            "`integrations/` subdirectory: external service integrations "
            "including Databricks, Linear, Sentry, and Snowflake. Backed by "
            "12 documentation files in the Phase 3 manifest (4 integration "
            "providers, each with README and supporting files, plus the "
            "docs/integrations/ subdirectory which documents some of these)."
        ),
        "scope_excludes": (
            "The AgentField project overview (covered by wiki/agentfield/agentfield.md), "
            "the AgentField ecosystem of separate repositories "
            ...
            "out of scope for this batch per user instruction. SDKs (Python, "
            "Go, TypeScript) are covered by a separate candidate "
            "(agentfield-sdks). Production deployment options are covered by "
            "a separate candidate (agentfield-deployments). Control plane "
            "internals and the top-level docs/ subdirectory are partially "
            "covered by the existing wiki/agentfield/agentfield.md note."
        ),
        "source_role": "core",
        "authority_level": "canonical-upstream",
        "lifecycle_status": "active",
...
BATCH_NAME = "agentfield-core"
BATCH_TITLE = "AgentField Core Candidate Batch"
...
def build_candidate(
    ...
    candidate = {
        ...
        "supersedes": None,
        "cssclasses": ["domain/agentfield", "layer/note", "state/candidate", f"agentfield/{cfg['source_role']}"],
        "source_type": "candidate-note",
        "topic": cfg["slug"],
        "topic_cluster": BATCH_NAME,
        "upstream_repo": source_id,
        "tags": ["agentfield", "candidate", "phase-5", "official", "core"],
...
def main():
    ...
    ap.add_argument("--generator-version", default="0.4.0")
    args = ap.parse_args()
    ...
    summary_path = candidates_dir / "_phase5-agentfield-core-summary.json"
