#!/usr/bin/env python3
"""Phase 5 — Knowledge Note Candidate Generator (n8n official/core batch).

Generates candidate knowledge notes under wiki/_candidates/ for sub-topics
of github:n8n-io/n8n-docs that are not already covered by the 14 existing
wiki notes under wiki/n8n/.

Sub-topics covered (all under github:n8n-io/n8n-docs):
  1. n8n-data-operations    — 27 files in n8n-docs/data/ (data flow, mapping,
                              structure, expressions, transformation)
  2. n8n-privacy-security   — 4 files in n8n-docs/privacy-security/ (GDPR,
                              telemetry, customer security practices)
  3. n8n-code-and-api       — 5 files: 4 in n8n-docs/code/ (Code node,
                              variables, builtin) + 1 in n8n-docs/api/
                              (REST API)

Excluded from this batch (per user instruction):
  - Community workflow/template repositories (Zie619/n8n-workflows,
    nusquama/n8nworkflows.xyz, wassupjay/n8n-free-templates,
...
Each candidate:
  ...
  - individual source_role=core (this batch is n8n official/core)
...
Usage:
    python3 _phase5_candidate_n8n_official.py \
...
# n8n official batch: 3 candidates, all from github:n8n-io/n8n-docs (1 core source)
# Each candidate has a file_path_filter to scope the evidence to specific subdirs.
N8N_OFFICIAL_SOURCES = {
...
# of n8n-docs/ to avoid overlap with the 14 existing wiki/n8n/*.md notes.
CANDIDATE_DEFS = [
    {
        "slug": "n8n-data-operations",
        "title": "n8n Data Operations — Data Flow, Mapping, and Transformation",
        "source_id": "github:n8n-io/n8n-docs",
        "file_path_filter": "n8n/data/",
        "scope_covers": (
            "Sub-topics of github:n8n-io/n8n-docs covered by the `docs/data/` "
            "subdirectory: how data flows between nodes, data structure, data "
            "mapping, expressions for data transformation, specific data types, "
            "and AI-assisted transformation. Backed by 27 documentation files "
            "in the Phase 3 manifest."
        ),
        "scope_excludes": (
            "n8n workflow concepts (covered by wiki/n8n/10-workflows.md), "
            "expressions and code in general (covered by wiki/n8n/13-expressions.md), "
            "hosting and self-hosted setup (wiki/n8n/30-hosting.md), integrations "
            "(wiki/n8n/35-integrations.md), user management "
            "(wiki/n8n/40-user-management.md), and the 14 existing wiki notes "
            "under wiki/n8n/. Community workflow repositories (Zie619, nusquama, "
            "wassupjay, enescingoz) and n8n-skills are out of scope for this "
            "batch."
        ),
        "source_role": "core",
        "authority_level": "canonical-upstream",
    ...
    {
        "slug": "n8n-privacy-security",
        "title": "n8n Privacy and Security — GDPR, Telemetry, and Customer Practices",
        "source_id": "github:n8n-io/n8n-docs",
        "file_path_filter": "n8n/privacy-security/",
        "scope_covers": (
            "Sub-topics of github:n8n-io/n8n-docs covered by the "
            "`docs/privacy-security/` subdirectory: n8n's privacy policies, "
            "GDPR data processing, documentation telemetry, what customers "
            "can do to improve security, and incident response. Backed by 4 "
            "documentation files in the Phase 3 manifest."
        ),
        "scope_excludes": (
            "Workflow-level security (covered by other notes; see the existing "
            "wiki/n8n/* notes), user management "
            "(wiki/n8n/40-user-management.md), and the 14 existing wiki notes "
            "under wiki/n8n/. Community workflow repositories (Zie619, nusquama, "
            "wassupjay, enescingoz) and n8n-skills are out of scope for this "
            "batch."
    ...
    {
        "slug": "n8n-code-and-api",
        "title": "n8n Code Node and Public API — JavaScript, Python, and REST",
        "source_id": "github:n8n-io/n8n-docs",
        "file_path_filter_multi": ["n8n/code/", "n8n/api/"],
        "scope_covers": (
            "Sub-topics of github:n8n-io/n8n-docs covered by the `docs/code/` "
            "and `docs/api/` subdirectories: the Code node for JavaScript and "
            "Python, custom variables, builtin methods and variables, and the "
            "n8n public REST API (authentication, pagination, the API playground, "
            "and endpoint reference). Backed by 5 documentation files in the "
            "Phase 3 manifest (4 in code/, 1 in api/)."
        ),
        "scope_excludes": (
            "General expressions and data flow (covered by wiki/n8n/13-expressions.md), "
            "core nodes and trigger nodes (covered by "
            "wiki/n8n/15-core-nodes.md and 16-trigger-nodes.md), and the 14 "
            "existing wiki notes under wiki/n8n/. Community workflow repositories "
            "(Zie619, nusquama, wassupjay, enescingoz) and n8n-skills are out "
...
BATCH_NAME = "n8n-official"
BATCH_TITLE = "n8n Official Candidate Batch"
...
def resolve_evidence(records: dict, source_id: str, cfg: dict) -> dict:
    """Resolve evidence for one candidate, filtered by file_path_filter or file_path_filter_multi."""
    ...
    # Filter occurrences for this source AND file path
    src_occs = [
...
def build_candidate(
    ...
    candidate = {
        ...
        "supersedes": None,
        "cssclasses": ["domain/n8n", "layer/note", "state/candidate", f"n8n/{cfg['source_role']}"],
        "source_type": "candidate-note",
        "topic": cfg["slug"],
        "topic_cluster": BATCH_NAME,
        "upstream_repo": source_id,
        "tags": ["n8n", "candidate", "phase-5", "official", "core"],
...
def main():
    ...
    ap.add_argument("--generator-version", default="0.3.0")
    args = ap.parse_args()
    ...
    summary_path = candidates_dir / "_phase5-n8n-official-summary.json"
