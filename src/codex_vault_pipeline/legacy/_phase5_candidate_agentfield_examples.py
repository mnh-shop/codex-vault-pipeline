#!/usr/bin/env python3
"""Phase 5 — Knowledge Note Candidate Generator (AgentField official examples batch).

Generates 7 candidate knowledge notes under wiki/_candidates/, one per
official-extension repository. The 7 repos are all from Agent-Field
(source_role: official-extension):

  1. SWE-AF          — Autonomous Engineering Team Runtime (272 occurrences)
  2. reels-af        — AI-Native Viral Reel Producer (1 occurrence)
  3. plandb          — PlanDB, the issue tracker for AI agents (19 occurrences)
  4. cloudsecurity-af — Cloud Infrastructure Security Scanner (6 occurrences)
  5. af-deep-research — Autonomous research backend (1 occurrence)
  6. pr-af           — Open-Source Agentic PR Reviewer (3 occurrences)
  7. sec-af          — AI-Native Security Auditor (2 occurrences)

One candidate per repository, preserving each repo's `source_role:
official-extension` (as recorded in the Layer A source record). No
subdivision: the user instruction allows subdivision only "unless
deterministic evidence supports a more useful subdivision"; for these
7 repos, 1 candidate per repo is the most useful structure (each repo
is a distinct, focused project).
...
Each candidate:
  ...
  - source_role: official-extension (preserved from Layer A)
...
Usage:
    python3 _phase5_candidate_agentfield_examples.py \
...
# 7 candidates, one per official-extension repository.
# slug and file_path_filter derived from the repo name (lowercased).
CANDIDATE_DEFS = [
    {
        "slug": "swe-af",
        "title": "SWE-AF — Autonomous Engineering Team Runtime (Agent-Field)",
        "source_id": "github:Agent-Field/SWE-AF",
        "file_path_filter": "swe-af/",
        "scope_covers": (
            "github:Agent-Field/SWE-AF at the snapshot preserved in "
            "codex-vault/raw/swe-af/. Covers the README, examples/ subdirectory "
            "(pyrust, diagrams, agent-comparison, llm-rust-python-compiler-sonnet), "
            "docs/ subdirectory (ARCHITECTURE, CONTRIBUTING, SKILL, deployment), "
            "and root-level files (CHANGELOG, CODE_OF_CONDUCT, SECURITY). "
            "Backed by 272 Phase 3 occurrences."
        ),
        "scope_excludes": (
            "The AgentField core repo (covered by wiki/agentfield/agentfield.md and "
            "the 3 AgentField core batch candidates). The 6 other AgentField "
            "official-extension repos (reels-af, plandb, cloudsecurity-af, "
            "af-deep-research, pr-af, sec-af) are out of scope for this "
            "candidate and each have their own dedicated candidate in this batch. "
            "Community workflow/template repos are not part of this Phase 5."
        ),
    },
    {
        "slug": "reels-af",
        "title": "REELS-AF — AI-Native Viral Reel Producer (Agent-Field)",
        "source_id": "github:Agent-Field/reels-af",
        "file_path_filter": "reels-af/",
        "scope_covers": (
            "github:Agent-Field/reels-af at the snapshot preserved in "
            "codex-vault/raw/reels-af/. Covers the README and its inline "
            "documentation. Backed by 1 Phase 3 occurrence."
        ),
        "scope_excludes": (
            "The AgentField core repo and the 6 other AgentField official-"
            "extension repos (each has its own dedicated candidate in this batch). "
            "Community workflow/template repos are not part of this Phase 5."
        ),
    },
    {
        "slug": "plandb",
        "title": "PlanDB — Issue Tracker for AI Agents (Agent-Field)",
        "source_id": "github:Agent-Field/plandb",
        "file_path_filter": "plandb/",
        "scope_covers": (
            "github:Agent-Field/plandb at the snapshot preserved in "
            "codex-vault/raw/plandb/. Covers the README, experiments/ "
            "subdirectory, examples/ subdirectory, docs/ subdirectory, and "
            "root-level files (CLAUDE, CONTRIBUTING, SECURITY). "
            "Backed by 19 Phase 3 occurrences. Note: the Layer A source "
            "record classifies plandb as `artifact_role: dataset` "
            "(different from the other example-agents which are "
            "`example-agent`); we preserve `source_role: official-extension` "
            "as recorded in the Layer A record."
        ),
        "scope_excludes": (
            "The AgentField core repo and the 6 other AgentField official-"
            "extension repos (each has its own dedicated candidate in this batch). "
    ...
    {
        "slug": "cloudsecurity-af",
        "title": "CloudSecurity AF — AI-Native Cloud Security Scanner (Agent-Field)",
        "source_id": "github:Agent-Field/cloudsecurity-af",
        "file_path_filter": "cloudsecurity-af/",
        "scope_covers": (
            "github:Agent-Field/cloudsecurity-af at the snapshot preserved in "
            "codex-vault/raw/cloudsecurity-af/. Covers the README, docs/ "
            "subdirectory, and benchmarks/ subdirectory. Backed by 6 Phase 3 "
            "occurrences."
        ),
        "scope_excludes": (
            "The AgentField core repo and the 6 other AgentField official-"
    ...
    {
        "slug": "af-deep-research",
        "title": "AF Deep Research — Autonomous Research Backend (Agent-Field)",
        "source_id": "github:Agent-Field/af-deep-research",
        "file_path_filter": "af-deep-research/",
        "scope_covers": (
            "github:Agent-Field/af-deep-research at the snapshot preserved in "
            "codex-vault/raw/af-deep-research/. Covers the README and its inline "
            "documentation. Backed by 1 Phase 3 occurrence. Note: the project "
            "is described as an 'Early Preview' in the README; APIs may change."
        ),
        "scope_excludes": (
            "The AgentField core repo and the 6 other AgentField official-"
            "extension repos (each has its own dedicated candidate in this batch). "
    ...
    {
        "slug": "pr-af",
        "title": "PR-AF — Open-Source Agentic PR Reviewer (Agent-Field)",
        "source_id": "github:Agent-Field/pr-af",
        "file_path_filter": "pr-af/",
        "scope_covers": (
            "github:Agent-Field/pr-af at the snapshot preserved in "
            "codex-vault/raw/pr-af/. Covers the README and the docs/ subdirectory. "
            "Backed by 3 Phase 3 occurrences."
        ),
        "scope_excludes": (
            "The AgentField core repo and the 6 other AgentField official-"
            "extension repos (each has its own dedicated candidate in this batch). "
    ...
    {
        "slug": "sec-af",
        "title": "SEC-AF — AI-Native Security Auditor (Agent-Field)",
        "source_id": "github:Agent-Field/sec-af",
        "file_path_filter": "sec-af/",
        "scope_covers": (
            "github:Agent-Field/sec-af at the snapshot preserved in "
            "codex-vault/raw/sec-af/. Covers the README and the docs/ subdirectory. "
            "Backed by 2 Phase 3 occurrences."
        ),
        "scope_excludes": (
            "The AgentField core repo and the 6 other AgentField official-"
            "extension repos (each has its own dedicated candidate in this batch). "
...
BATCH_NAME = "agentfield-examples-official"
BATCH_TITLE = "AgentField Official Examples Candidate Batch"
...
def resolve_evidence(records: dict, source_id: str, cfg: dict) -> dict:
    """Resolve evidence for one candidate, filtered by file_path_filter."""
    artifacts = records["artifacts"]
    occurrences = records["occurrences"]
    units = records["units"]

    filters = []
    if "file_path_filter" in cfg:
        filters.append(cfg["file_path_filter"])
    if "file_path_filter_multi" in cfg:
        filters.extend(cfg["file_path_filter_multi"])

    def path_matches(sp: str) -> bool:
        return any(sp.startswith(f) for f in filters)

    src_occs = [
        o for o in occurrences
        if o.get("source_id") == source_id and path_matches(o.get("source_path", ""))
    ]
...
def build_candidate(
    ...
    # Use the precise source_role from Layer A (all 7 are official-extension)
    source_role = layer_a.get("source_role") or "official-extension"
    authority_level = layer_a.get("authority_level") or "official"
    lifecycle_status = layer_a.get("lifecycle_status") or "active"
    ...
    candidate = {
        ...
        "supersedes": None,
        "cssclasses": ["domain/agentfield", "layer/note", "state/candidate", f"agentfield/{source_role}"],
        "source_type": "candidate-note",
        "topic": cfg["slug"],
        "topic_cluster": BATCH_NAME,
        "upstream_repo": source_id,
        "tags": ["agentfield", "candidate", "phase-5", "official", "official-extension", "example-agent"],
...
def build_migration_report(
    ...
    body = {
        ...
        "promotion_blockers": [
            ...
            "Existing wiki notes under wiki/agentfield/ are not modified (per requirement)",
        ],
        "evidence_summary": {
            "source_id": source_id,
            "layer_a_source_record_id": layer_a["record_id"],
...
def main():
    ...
    ap.add_argument("--generator-version", default="0.5.0")
    args = ap.parse_args()
    ...
    summary_path = candidates_dir / "_phase5-agentfield-examples-summary.json"
